...
import { useAIRecommendations } from '@mystacks/hooks';
...
export const RecommendationsPage = (props: RecommendationsPageProps) => {
  ...
  const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const affectedBook = savedBooks.find(
      (book) => book.id == event.target.name
    );
    if (event.target.checked && affectedBook) {
      setInputBooks([...inputBooks, affectedBook]);
    } else {
      setInputBooks(
        inputBooks.filter((inputBook) => inputBook.id != event.target.name)
      );
    }
  };
        ...
        <TitleContainer id="1">
          ...
          >
            <Typography
              ...
              id="typedtext2"
            >
              Select the books you would like to use as input for the AI
              recommendations
            </Typography>
          </Box>
        </TitleContainer>
                    ...
                    <FormControlLabel
                      control={
                        <Checkbox
                          checked={
                            !!inputBooks.filter(
                              (inputBook) => inputBook.id == book.id
                            ).length
                          }
