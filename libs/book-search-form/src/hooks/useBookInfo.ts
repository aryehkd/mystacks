...
export const useBookInfo = (appState:  AppStateType, book?: Book, bookId?: string) => {
    ...
    const loadBook = (bookId: string) => {
        try {
            ...
            const url = "https://getstoredbook-u6erzcpcda-uc.a.run.app?" + new URLSearchParams({
                  userId: userId,
                  bookId: bookId
              })
            ...
            request(url, requestOptions)
                ...
                .then(result => {
                    ...
                    setBookProgress(loadedBook.userRating?.bookProgress || BookProgressStates.ToRead)
                    setRating(loadedBook.userRating?.rating || 0)
                    setNotes(loadedBook.userRating?.notes || '')
                    setISBN(loadedBook.bookInfo?.industryIdentifiers?.isbn13 || loadedBook.bookInfo?.industryIdentifiers?.isbn10 || '')
                    setBookLoaded(true)
                })
                .catch(error => console.log('error', error) // TODO: add handling here to display not found
                );
          } catch (e) {
            console.error(e)
          }
